...
function DisplayOutputAlt {
    ...
    Write-Host "`t+++`twindowsVersion`twindowsEdition`tipAddress`t CurrentDomain`t computerName`t defaultGateway`t osSupported`t LastGPOAppliedTime`t lastSecurityUpdate`t localGuestAccountEnabled`t LocalAdmins`t FirewallServiceRunning`t firewallStatusDomain`t firewallStatusPrivate`t firewallStatusPublic`t ChromeInstalled`t chromeVersion`t FirefoxInstalled`t firefoxVersion`t EdgeInstalled`t msedgeVersion`t IEInstalled`t ieVersion`t AVEnabled2`t AVOnAccess`t AVRealTimeProtectionEnabled`tAVSignatureAge`t"
